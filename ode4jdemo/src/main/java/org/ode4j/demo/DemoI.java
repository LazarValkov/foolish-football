...
//			dMultiply0 (v.v,R.v,q.v,i*3,3,3,1);
	...
	private void simLoop (boolean pause)
	{
		...
		dsSetColor (1,1,1);
		dsDrawBox (anchor_body.getPosition(), anchor_body.getRotation(), sides3);
		dsSetColor (1,0,0);
		dsDrawBox (test_body.getPosition(), test_body.getRotation(), sides2);
		dsSetColor (1,1,0);
		for (int i=0; i<NUM; i++)
			dsDrawBox (particle[i].getPosition (), particle[i].getRotation (), sides);
	}
